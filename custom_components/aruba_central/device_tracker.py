...
CONF_SCAN_INTERVAL = "scan_interval"  # throttle Central API (sec or HH:MM:SS)
...
def _parse_scan_interval(val) -> int:
    ...
    if isinstance(val, str):
        ...
        try:
            ...
            _LOGGER.error("aruba_central(DeviceScanner): invalid scan_interval '%s', using default %ss",
...
async def async_get_scanner(hass: HomeAssistant, config: dict) -> Optional[DeviceScanner]:
    ...
    _LOGGER.warning("aruba_central(DeviceScanner): scan_interval(min)= %ss", min_interval_s)
...
class _CentralAPI:
    ...
    async def _ensure_token(self):
        if self.access_token and time.time() < self.expiry - 60:
            _LOGGER.warning("aruba_central(DeviceScanner): using cached token (expires_in=%ss)", int(self.expiry - time.time()))
            return
...
class ArubaCentralScanner(DeviceScanner):
    def __init__(self, *, hass: HomeAssistant, session: aiohttp.ClientSession, api_base: str, oauth_base: str,
                 client_id: str, client_secret: str, refresh_token: str, customer_id: Optional[str],
    ...
    async def async_scan_devices(self) -> List[str]:
        ...
        for mac in offline_macs:
            _LOGGER.warning("aruba_central(DeviceScanner): %s → not_home (missing in latest Central list)", mac)
